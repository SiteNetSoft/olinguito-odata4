...
/**
 * Quarkus deployment processor for the OData server extension.
 * Registers per-service routes and marks OData beans as unremovable.
 */
public class ODataProcessor {
    ...
    /**
     * Creates the OData build item with configuration for all named services.
     */
    @BuildStep
    ODataBuildItem createBuildItem(ODataServicesBuildTimeConfig config) {
        Map<String, ODataBuildItem.ServiceEntry> services = new LinkedHashMap<>();
        for (var entry : config.services().entrySet()) {
            String name = entry.getKey();
            var serviceConfig = entry.getValue();
            String path = serviceConfig.path().orElseGet(() ->
                    ODataServiceNames.isDefault(name) ? "/odata" : "/" + name);
            services.put(name, new ODataBuildItem.ServiceEntry(serviceConfig.enabled(), path));
        }
        // Ensure the default service always exists
        services.putIfAbsent(ODataServiceNames.DEFAULT,
                new ODataBuildItem.ServiceEntry(true, "/odata"));
        return new ODataBuildItem(services);
    }

    /**
     * Marks OData bean types as unremovable so they are available
     * for qualifier-aware lookup from the recorder.
     */
    @BuildStep
    void markODataBeansUnremovable(BuildProducer<UnremovableBeanBuildItem> unremovable) {
        unremovable.produce(UnremovableBeanBuildItem.beanTypes(CsdlEdmProvider.class));
        unremovable.produce(UnremovableBeanBuildItem.beanTypes(Processor.class));
        unremovable.produce(UnremovableBeanBuildItem.beanTypes(EdmxReference.class));
    }
    ...
    /**
     * Registers OData routes for each configured service at runtime.
     * Each service gets an exact-path route and a wildcard route.
    ...
    void registerRoutes(
        ...
        for (var entry : odataBuildItem.getServices().entrySet()) {
            String serviceName = entry.getKey();
            ODataBuildItem.ServiceEntry service = entry.getValue();

            if (!service.enabled()) {
                continue;
            }

            String basePath = service.path();
            // Normalize: remove trailing slash if present
            if (basePath.endsWith("/") && basePath.length() > 1) {
                basePath = basePath.substring(0, basePath.length() - 1);
            }

            // Wildcard route for all OData sub-paths (e.g., /odata/*)
