...
As a shortcut you should checkout the prepared tutorial project in the [git repository](https://github.com/sitenetsoft/olinguito-odata4) in folder /samples/tutorials/p12_deep_insert_preparation.
...
Before we start with the implementation, please have a look at the class `myservice.mynamespace.data.Storage`. In difference to the [navigation tutorial](navigation.md) the relations between two entities can not be hard coded because we would like to create and change relations between entities dynamically. In the constructor of the data storage the creation of the sample data is called. After that the method `linkProductsAndCategories`is called. This methods sets a few links between the just created entities. **The linked entites are stored as navigation links**

To express the relation between two entities, Olinguito uses the class `Link`. This class is used for related entites (directly connected via Java references) and bindings (which are actually strings) to other entities. To get the related entites for a particual navigation property, you can ask an entity with the method `getNavigationLink(String name)` for an navigation property link. The link will contain either an entity or a collection of entities dependenting on the type of the navigation property. To get the actual entities use the methods `getInlineEntity()` or `getInlineEntitySet()`.
...
![Deserializer Result](../../images/tutorials/deep_insert/before.png)

When our implementation has processed the whole request, all entites are created and linked as **navigationLinks**.

![After Deep insert](../../images/tutorials/deep_insert/after.png)
...
![Entity Set Categories](../../images/tutorials/deep_insert/categories_entity_set.png)
...
![Related products for category "Food"](../../images/tutorials/deep_insert/related_products.png)
...
Further topics to be covered by follow-up tutorials:
  ...
  * Tutorial OData V4 service, part 7: [Add Media entities to the service](media.md)
