# How to build an OData Service with Olinguito V4

## Part 1: Read scenario

This tutorial guides you through the steps required to write an OData Service based on the Olinguito OData 4.0 Library for Java (based on current olingo version which can be got via the [Download-Page](https://github.com/sitenetsoft/olinguito-odata4/releases)).

The final source code can be found in the project [git repository](https://github.com/sitenetsoft/olinguito-odata4).
A detailed description how to checkout the tutorials can be found [here](../prerequisites.md).
...
![datamodel](../../images/tutorials/read/model1.png  "The OData model")
...
![projectLayout](../../images/tutorials/read/EclipseProjectTree.png  "The project layout")
...
![eclipseDownload](../../images/tutorials/read/eclipseDownload.png  "The Eclipse EE download")
...
![mavenArchetype](../../images/tutorials/read/mavenArchetype.png  "The Maven Archetype")
...
In our project, we will be using several libraries, e.g. the Olinguito libraries.
...
![ConfigureBuildPathAction](../../images/tutorials/read/ConfigureBuildPathAction.png  "Open the Configure BuildPath action")

Select the *Source* tab.
You might see that the source folder *src/main/java* is configured, but displays an error marker.

![ConfigureBuildPathErr](../../images/tutorials/read/ConfigureBuildPathErr.png  "Configure BuildPath has errors")

The reason is that it is missing on file system.
So the solution is to create the required folder in Eclipse.

![createFolder](../../images/tutorials/read/createFolder.png  "Create new java folder")
...
![ConfigureBuildPathOK](../../images/tutorials/read/ConfigureBuildPathOK.png  "Configure Build Path is OK now")
...
![mavenBuild](../../images/tutorials/read/mavenBuild.png  "The Maven build dialog")
...
The implementation of an OData service based on Olinguito server library can be grouped in the following steps:
...
Olinguito provides an API for it and we will use it in the implementation of our *CsdlEdmProvider*.


### 4.1.2. Create class

Create package *myservice.mynamespace.service*
Create class *DemoEdmProvider* and specify the superclass *org.sitenetsoft.olinguito.commons.api.edm.provider.CsdlAbstractEdmProvider*
...
As we can see, the Olinguito server API provides one package that contains interfaces for the description of the metadata:

![edmPackage](../../images/tutorials/read/edmPackage.png  "The edm package")

Some of these interfaces are going to be used in the following sections.
**Note:** Take a look into the [Javadoc](https://github.com/sitenetsoft/olinguito-odata4)
...
![overrideMethods](../../images/tutorials/read/overrideMethods.png  "The dialog for overriding superclass methods in Eclipse")
...
We have declared the main elements of an OData service: _EntityType_, _EntitySet_, _EntityContainer_ and _Schema_ (with the corresponding Olinguito classes `CsdlEntityType`, `CsdlEntitySet`, `CsdlEntityContainer` and `CsdlSchema`).
...
In terms of _Olinguito_, while processing a service request, a Processor instance is invoked that is supposed to understand the (user HTTP-) request and deliver the desired data.
_Olinguito_ provides API for processing different kind of service requests:
...
So, whenever this URL is fired, Olinguito will invoke the `EntityCollectionProcessor` implementation of our OData service.
...
Within our package `myservice.mynamespace.service`, we create a Java class `DemoEntityCollectionProcessor`  that implements the interface `org.sitenetsoft.olinguito.server.api.processor.EntityCollectionProcessor`.

![createJavaClass](../../images/tutorials/read/createJavaClass.png  "The Eclipse dialog for creating a Java class")
...
After creation of the Java class, we can see that there are 2 methods to be implemented:
  ...
  * _init()_
    This method is invoked by the _Olinguito_ library, allowing us to store the context object
  * _readEntityCollection()_
    Here we have to fetch the required data and pass it back to the _Olinguito_ library
...
The _Olinguito_ framework initializes the processor with an instance of the _OData_ object.
...
The steps for implementating the method `readEntityCollection(...)` are:
  ...
  2. Fetch the data
    ...
    The package `org.sitenetsoft.olinguito.commons.api.data` provides interfaces that describe the actual data, not the metadata.

      ![datapackage](../../images/tutorials/read/datapackage.png  "The package containing the interfaces for handling runtime data")

  3. Transform the data
    _Olinguito_ expects us to provide the data as low-level _InputStream_ object. However, _Olinguito_ supports us in doing so, by providing us with a proper "serializer".
...
The servlet is a standard _HttpServlet_ which dispatches the user requests to the _Olinguito_ framework.
...
![createJavaServletClass](../../images/tutorials/read/createJavaServletClass.png  "Creating the servlet class")

Override the `service()` method.
Basically, what we are doing here is to create an `ODataHttpHandler`, which is a class that is provided by _Olinguito_.
...
![runOnServer](../../images/tutorials/read/runOnServer.png  "The Eclipse dialog for deploying web apps to a server")
...
Finally, we have created our first OData service based on the V4 version of the OData specification and using the V4 server library provided by _Olinguito_.
...
The final source code result can be found in the project [git repository](https://github.com/sitenetsoft/olinguito-odata4).
A detailed description how to checkout the tutorials can be found [here](../prerequisites.md).
...
Further topics to be covered by follow-up tutorials:
  ...
  * Tutorial OData V4 service part 2: [Read Entity, Read Property](readep.md)
  * Tutorial OData V4 service part 3: [Write (Create, Update, Delete Entity)](write.md)
  * Tutorial OData V4 service, part 4: [Navigation](navigation.md)
  * Tutorial OData V4 service, part 5.1: [System Query Options $top, $skip, $count (this page)](sqo-tcs.md)
  * Tutorial OData V4 service, part 5.2: [System Query Options $select, $expand](sqo-es.md)
  * Tutorial OData V4 service, part 5.3: [System Query Options $orderby](sqo-o.md)
  * Tutorial OData V4 service, part 5.4: [System Query Options $filter](sqo-f.md)
  * Tutorial ODATA V4 service, part 6: [Action and Function Imports](action.md)
  * Tutorial ODATA V4 service, part 7: [Media Entities](media.md)
  * Tutorial OData V4 service, part 8: [Batch Request support](batch.md)
  * Tutorial OData V4 service, part 9: [Handling "Deep Insert" requests](deep-insert.md)
  
### Code and Repository
  * [Git Repository](https://github.com/sitenetsoft/olinguito-odata4)
  * [Guide - To fetch the tutorial sources](../prerequisites.md)
  * [Demo Service source code as zip file (contains all tutorials)](https://github.com/sitenetsoft/olinguito-odata4/tree/master/samples)

### Further reading

  * [Official OData Homepage](http://odata.org/)
  * [OData documentation](http://www.odata.org/documentation/)
  * [Olinguito Javadoc](https://github.com/sitenetsoft/olinguito-odata4)
