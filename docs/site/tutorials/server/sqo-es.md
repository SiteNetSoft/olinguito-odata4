...
Same prerequisites as in [Tutorial Part 4: Navigation](navigation.md) as well as basic knowledge about the concepts presented there.

Furthermore, basic knowledge about *System Query Options* (see [Tutorial Part 5.1](sqo-tcs.md)) is helpful.

___

# 2. Preparation

Follow [Tutorial Part 4: Navigation](navigation.md) or as shortcut import  *Part 4: Navigation* into your Eclipse workspace.
...
![AllProductsNoQueryOption](../../images/tutorials/sqo_es/responseProducts_Full.jpg  "The full list of Products")
...
![AllProductsSelectName](../../images/tutorials/sqo_es/responseProducts_SelectName.jpg  "The full list of Products, but displayling only the Name property")
...
The Olinguito library parses the `$select` option from the request and provides support in the serializer to serialize only the selected properties.  
...
In this tutorial we use the simple implementation to show the concept for `$select` so that there are only a few steps that have to be done by us.  
  ...
  * We have to take care about the context URL, which is different in case that `$select` is used.
    Again, the Olinguito library provides some support, which we use to build the select list that has to be passed to the ContextURL builder:
...
In order to understand the `$expand` system query option, let’s first quickly recap what we’ve learned in the navigation-tutorial:
  ...
  1. In order to be able to navigate from one entity to another entity, we need at least 2 EntityTypes and at least one NavigationProperty  
      ![Metadata](../../images/tutorials/sqo_es/metadataNav.jpg  "Declaring the navigation in the metadata")

  1. We can invoke one single entity, e.g. display one product:  
      ![Products(1)SingleRead](../../images/tutorials/sqo_es/responseProducts_1.jpg  "The result of a single read")
  ...
  As we’ve seen in the metadata above, the name of the navigation property is *Category*  
      ![Products(1)NavToCat](../../images/tutorials/sqo_es/responseProducts_1_navCat.jpg  "Navigating from a Product to its Category")
...
![Products(1)Expanded](../../images/tutorials/sqo_es/responseProducts_1_expandCat.jpg  "The Product with its expanded Category")

One more advantage is that the system query option `$expand` can also be applied to an entity collection:

![AllProductsExpanded](../../images/tutorials/sqo_es/responseProducts_expandCat.jpg  "All Products with expanded Category")
...
```java
    if(expandItem.isStar()) {
      ...
      if(!bindings.isEmpty()) {
        ...
        // we don't need to handle error cases, as it is done in the Olinguito library
...
**Note:**  
The complete `readEntity(...)` method can be found in the *Appendix* at the end of the site or together with the `readEntityCollection(...)` method in the [sample project zip](https://github.com/sitenetsoft/olinguito-odata4/tree/master/samples), [sha512], [pgp]).
...
In this tutorial we have learned the basics of the `$select` and `$expand` *system query options* within the OData context as well as how to implement those features with the *Olinguito library* by using the provided convenience and support methods.
...
### Tutorials
  * Tutorial OData V4 service part 1: [Read Entity Collection](read.md)
...
```java
    public void readEntity(ODataRequest request, ODataResponse response, UriInfo uriInfo, ContentType responseFormat)
      ...
      // handle $select
      SelectOption selectOption = uriInfo.getSelectOption();
      // in our example, we don't have performance issues, so we can rely upon the handling in the Olinguito lib
