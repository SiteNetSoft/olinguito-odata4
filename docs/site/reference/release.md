# Olinguito Release Documentation

---

### Introduction

This document describes the release guidelines for Olinguito. It heavily refers
...
Olinguito is built and released with [Maven 3](https://maven.apache.org) and uses
the <a href="https://svn.apache.org/repos/asf/maven/pom/tags/apache-16/pom.xml">Apache POM version 16</a>.

### Release Artifacts

An Olinguito release consists of:

  * The main POMs/JARs/WARs built as part of the Maven build process.
    For an overview on the released modules see artifacts with *groupId*: `org.sitenetsoft.olinguito` in the [Apache Maven Repository](https://repository.apache.org/index.html#nexus-search;gav~org.sitenetsoft.olinguito~~4.0.0~~).
...
provided as part of the release:
  ...
  *  `Olinguito-OData-${version}-source-release.${ext}` <br/> A source-release
bundle containing all files the sources necessary to build all other artifacts. <br/> **Package formats**: zip.
  *  `Olinguito-OData-JavaDoc-${version}-javadoc.${ext}` <br/> A bundle
...
examples. <br/> **Package formats**: zip.
  *  `Olinguito-OData-Server-for-Java-${version}-lib.${ext}` <br/> A bundle containing the OData4 core
library and dependencies required to implement an OData V4 Java server/processor. <br/> **Package formats**: zip.
  *  `Olinguito-OData-Client-for-Java-${version}-jpa.${ext}` <br/> A bundle containing the OData4 client library
  and dependencies required to implement an OData V4 Java client. <br/> **Package formats**: zip.
  *  `Olinguito-OData-Client-for-Android-${version}-lib.${ext}` <br/> A bundle containing the OData4 client library
...
up-to-date. Release independend documentation is maintained on the [Olinguito Documentation][1] page.
...
The Olinguito community decides if the release will be a major or a minor release and
...
There must not be any open issues for this release. There might be open issues for
...
code should follow the Olinguito code style. For verification run following
Maven execution
...
made available [here](https://repository.apache.org/index.html#nexus-search;gav~org.sitenetsoft.olinguito~~~~).
  - Publish final release Version to [Apache Repository](https://repository.apache.org/)
    ...
    - Afterwards the Maven artifacts are automatically synced to [Maven Central](https://search.maven.org/#search|ga|1|org.sitenetsoft.olinguito).
...
signatures) to [Apache Distributions](https://www.apache.org/dist/olingo/).
  ...
  - Release is closed in the issue tracker.
  - Release is announced to the project mailing list.
...
To maintain the released Distributions for the download pages following steps are required (for more information see [Apache Distribution Documentation](https://www.apache.org/dev/release-publishing.html#distribution_dist)) to upload the new distribution via SVN (`svn co https://dist.apache.org/repos/dist/release/olingo`):
  ...
  - Change into the directory according to the released Olinguito artifact (e.g. `odata2`, `odata4`, ...)
  ...
  - Add new directory (e.g. `svn add rel-x.x.x`) and do the commit (e.g `svn ci -m "Added Olinguito x.x.x release"`)
...
### Additional Release Information
  ...
  [1]: https://github.com/sitenetsoft/olinguito-odata4
  [2]: https://github.com/sitenetsoft/olinguito-odata4/issues
