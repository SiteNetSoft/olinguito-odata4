...
- **Recorded deviations:**
  ...
  39. **`ODataXmlSerializer.checkNoStreamProperty` walks declared and complex properties but not navigation properties**, so a streamed XML collection with `$expand` onto a stream-bearing type still hits the mid-stream refusal rather than the clean pre-commit 400. The pre-commit check is therefore a best-effort narrowing of the mid-stream failure, not an exhaustive one.
  40. **The known cross-thread hazard when a handler instance is shared across requests is wider than the `uriInfo` read of deviation 34/j**: the asynchronous thread also writes `ODataHandlerImpl.lastThrownException` and, in debug mode, appends to `ServerCoreDebugger.runtimeInformation`, a plain `ArrayList`. Not reachable in the servlet deployment, where the handler is created per request.
  41. **`TechnicalAsyncService.resolve` keys strictly on `getRawRequestUri()`**, so a monitor GET carrying a query string or a trailing slash answers 404 instead of the result. Reference service only; the SPI itself puts no constraint on how a service identifies its monitors.
  42. **`TechnicalAsyncService.isStatusMonitorRequest` is a `/status/` substring test** now applied by the main OData servlet to every request, so any OData URL containing that literal is diverted to the monitor branch. Reference service only, and the tecsvc EDM has no such name.
  43. **`AsyncRunner` marks the invocation finished when `invoke()` returns**, but a streamed result's body is rendered later, when the monitor serves it. For streamed responses the work is therefore not truly deferred, and a `SerializerException` thrown at that late point has no error path — it surfaces on the monitor response, not on the asynchronous result.
