...
abstract class AbstractInvocationHandler implements InvocationHandler {
  ...
  protected
  <S extends org.sitenetsoft.olinguito.ext.proxy.api.EntityType<?>> void
  deleteEntity(final EntityInvocationHandler handler, final URI entitySetURI) {
    ...
    final String name = handler.getUUID().getType().
        getAnnotation(EntityType.class).name();
